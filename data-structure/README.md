...
- 해시 충돌 해결 방법
    ...
    1. 개방 주소 방식
        ...
        - 단점
            ...
            - 제곱 탐사는 2차 군집화 현상이 발생할 수 있음 (선형 탐사 단점은 완화)
            - 이중 해싱은 구현이 복잡하며, 개방 주소 방식의 공통적인 한계로 더미 마커를 사용해야 하고, 적재율(Load Factor, α)이 1에 가까워질수록 성능이 급격히 낮아질 수 있음 (제곱 탐사의 단점은 완화)
- 리해싱
    - 일반적으로 기존 bucket 크기의 2배가량으로 늘리면서 Hash 함수의 로직을 변경함
    - 일반적으로 적재율(Load Factor, α)의 임계점이 낮은 순서대로 빨리 일어남 (더미 마커 비율 등 다른 요인도 존재)
