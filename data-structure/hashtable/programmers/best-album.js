...
//      - 모든 장르는 재생된 횟수가 다름
// 2. 구현 방법
//      - Map에 장르를 key로 하고 총 재생 횟수를 value로 하는 데이터를 저장 후 정렬
//      - Map에 장르를 key로 하고 고유 번호, 재생 횟수를 value로 하는 데이터를 저장
//      - 정렬된 장르 수만큼 Loop를 돌면서 장르 내에서 재생 횟수가 많은 노래부터 수록, 재생 횟수가 같으면 고유 번호가 낮은 노래부터 수록, 장르별 최대 2개만 수록
function solution(genres, plays) {
  ...
  plays.forEach((play, idx) => {
    if (mapOfMusics.has(genres[idx])) {
      mapOfMusics.set(genres[idx], [
        ...mapOfMusics.get(genres[idx]),
        { number: idx, playCount: play },
      ]);
    } else {
  ...
  sortedListOfGenre.forEach((genre) => {
    ...
    album.push(
      ...listOfMusicAboutOneGenre.slice(0, 2).map((music) => music.number)
    );
  });
